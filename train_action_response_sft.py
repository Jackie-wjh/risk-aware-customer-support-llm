...
def parse_model_json(text):
    ...
    This avoids regex problems when customer_response contains text like:
    "{Order Number}" or other brace-like placeholders.
    """
...
def extract_action_and_response(text):
    ...
    json_parse_success:
        True only when a valid JSON object is parsed and action is valid.

    response_parse_success:
        True only when JSON is valid, action is valid, and customer_response exists.
    """
    text = str(text or "").strip()
...
def calculate_extra_metrics(df):
    ...
    metrics = {
        ...
        # unsafe answer count divided by all evaluation rows.
...
def build_summary(df, report_dict, accuracy, macro_f1, extra):
    summary = {
        "accuracy": accuracy,
        "macro_f1": macro_f1,
...
def save_eval_visuals(df, cm_df, output_path, report_dict, extra):
    ...
    plt.ylabel("True")
    plt.tight_layout()
    plt.savefig(visuals_dir / "confusion_matrix.png", dpi=200)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(visuals_dir / "per_class_metrics.png", dpi=200)
    plt.close()
    ...
    plt.ylabel("Count")
    plt.tight_layout()
    plt.savefig(visuals_dir / "prediction_distribution.png", dpi=200)
    plt.close()

    # 4. unsafe_metrics.png
    unsafe_items = {
    ...
    plt.tight_layout()
    plt.savefig(visuals_dir / "unsafe_metrics.png", dpi=200)
    plt.close()
...
def main():
    ...
    # Stage 1B response output needs enough tokens.
    ...
    for start_idx, batch_rows in batch_iter(rows, args.batch_size):
        ...
        for local_i, (ex, pred_text) in enumerate(zip(batch_rows, pred_texts)):
            ...
            sample_id = ex.get("sample_id", row_idx)
            ...
            results.append({
                ...
                # Original code-style labels
                "gold": gold,
                "pred": pred_action,

                # Standard report-style labels
                "true_decision": LABEL_TO_STANDARD.get(gold, gold),
                "pred_decision": LABEL_TO_STANDARD.get(pred_action, pred_action),
                ...
                "pred_response": parsed["customer_response"],
    ...
    summary = build_summary(df, report_dict, accuracy, macro_f1, extra)
    ...
    confusion_path = output_path.parent / "confusion_matrix.csv"
    ...
    run_config = {
        ...
        "label_to_standard": LABEL_TO_STANDARD,
